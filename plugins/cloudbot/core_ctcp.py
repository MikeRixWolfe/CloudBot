...
async def ctcp_version(notice, irc_ctcp_text):
    if irc_ctcp_text:
        ...
        if command == 'VERSION':
            notice("\x01VERSION Gary a fork of Cloudbot {} - https://github.com/MikeRixWolfe/CloudBot\x01".format(
                cloudbot.__version__))
        elif command == 'PING':
            notice('\x01PING {}\x01'.format(params))  # Bot should return exactly what the user sends as the ping parameter
        elif command == 'TIME':
